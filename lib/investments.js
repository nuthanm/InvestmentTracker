...
export function getTransactionTypeLabel(type) {
  return MARKET_TRANSACTION_TYPES.find((item) => item.value === type)?.label || type;
}
...
export function attachInvestmentSummaries(investments = [], transactions = []) {
  ...
  return investments.map((investment) => {
    if (!isMarketInvestment(investment.type_code)) return investment;
    return {
      ...investment,
...
export function effectiveInvestedAmount(investment) {
  if (isMarketInvestment(investment.type_code)) {
...
export function effectiveInvestedSoFar(investment) {
  ...
  if (frequency === 'monthly') {
    ...
    monthsElapsed = Math.max(0, Math.min(monthsElapsed, tenureMonths));
  ...
  if (today >= anniversaryThisYear) yearsElapsed += 1;
  yearsElapsed = Math.max(0, Math.min(yearsElapsed, years));
