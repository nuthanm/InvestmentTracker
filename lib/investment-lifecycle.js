...
import { isMarketInvestment, isMetalInvestment, isTransactionBased, isChitInvestment } from '@/lib/investments';
...
export function computePrematureClosurePreview(investment, options = {}, summary = null) {
  ...
  if (isChitInvestment(investment.type_code)) {
    const asOf = options.closureDate || investment.closure_date || new Date().toISOString().slice(0, 10);
    const investedPrincipal = investedPrincipalUntil(investment, asOf, options.investedOverride);
    const prize = Number(investment.maturity_value || investment.chit_details?.summary?.prize || 0);
    const pickMonth = Number(investment.chit_details?.pick_month || 0);
    const start = startOfDay(investment.start_date);
    const closure = startOfDay(asOf);
    let elapsed = 0;
    if (start && closure && closure >= start) {
      const monthsDiff = (closure.getFullYear() - start.getFullYear()) * 12 + (closure.getMonth() - start.getMonth());
      const dueThisMonth = new Date(start);
      dueThisMonth.setMonth(dueThisMonth.getMonth() + monthsDiff);
      elapsed = Math.max(0, monthsDiff + (dueThisMonth <= closure ? 1 : 0));
    }
    const received = elapsed >= pickMonth && pickMonth > 0 ? prize : investedPrincipal;
    const penaltyAmount = Number(options.penaltyAmount || 0);
    const netReceived = roundMoney(Math.max(0, received - penaltyAmount));
    return {
      kind: 'chit',
      investedPrincipal: roundMoney(investedPrincipal),
      monthsHeld: monthsElapsedBetween(investment.start_date, asOf),
      daysHeld: daysHeldBetween(investment.start_date, asOf),
      interestAtContractedRate: 0,
      interestAtEffectiveRate: 0,
      interestLoss: 0,
      penaltyAmount: roundMoney(penaltyAmount),
      netReceived,
      maturityValue: prize,
      valueAtContractedRate: roundMoney(received),
      valueAtEffectiveRate: roundMoney(received),
    };
  }
