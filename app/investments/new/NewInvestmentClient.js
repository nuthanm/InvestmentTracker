...
const TYPES = ['FD', 'RD', 'MF', 'ETF', 'ST', 'GD', 'PPF', 'OT'];
const PRESET_TYPES = ['Chit Fund', 'Crypto', 'Real Estate', 'NSC', 'Sukanya Samriddhi', 'Lent to family'];

const TYPE_CHIP_LABEL = { FD: 'FD', RD: 'RD', MF: 'MF', ETF: 'ETF', ST: 'Shares', GD: 'GD', PPF: 'PPF', OT: 'Other' };
const TYPE_TOOLTIP = {
...
export default function NewInvestmentClient({
  ...
  }, [initialCharges, initialPrice, initialTaxes, initialUnits, isMarketType]);
  ...
  const submit = async (e) => {
    ...
    if (isMarketType) {
      ...
      if (hasAnyInitialInput) {
        ...
        const charges = Number(initialCharges || 0);
        const taxes = Number(initialTaxes || 0);
        if (units <= 0) { setError('Initial buy units must be greater than zero.'); return; }
        if (price <= 0) { setError('Initial buy price must be greater than zero.'); return; }
        if (charges < 0 || taxes < 0) { setError('Charges and taxes cannot be negative.'); return; }
        initialTransaction = {
      ...
      <div className="px-4 md:px-8 py-5 md:py-6 max-w-2xl mx-auto w-full">
        ...
        <form onSubmit={submit} className="space-y-5">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input type="text" placeholder={isMarketType ? 'Nifty ETF / SIP folio / Company name' : typeCode === 'RD' ? 'Monthly RD - 2Y' : typeCode === 'PPF' ? 'PPF Account 2024' : 'Senior FD - 5Y'} value={planName} onChange={(e) => setPlanName(e.target.value)} className="field-input" />
              </div>
            </div>
          </section>
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-ink-soft mb-1.5">{isMarketType ? 'Holding start / first trade date' : 'Start / debit date'}<span className="text-danger ml-0.5">*</span></label>
              <input type="date" value={startDateInput} onChange={(e) => setStartDateInput(e.target.value)} className="field-input" />
              <p className="text-[11px] text-ink-mute mt-1">{isMarketType ? 'Used as the opening date for this holding and the default date for the first buy.' : 'For monthly/yearly plans, this date sets the recurring debit day anchor.'}</p>
            </div>
          </section>

          {isMarketType ? (
            <section className="space-y-3">
              <p className="text-[11px] tracking-wider text-ink-mute uppercase">Initial buy <span className="text-[10px] normal-case ml-1">optional</span></p>
              {isEditing ? (
                <div className="rounded-xl border border-edge bg-paper-tint p-3 text-[12px] text-ink-soft">Use the investment detail page to add another buy, redeem units, or record dividends.</div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs text-ink-soft mb-1.5">Units</label>
                    <input type="number" step="0.000001" placeholder="12.500000" value={initialUnits} onChange={(e) => setInitialUnits(e.target.value)} className="field-input" />
                  </div>
                  <div>
                    <label className="block text-xs text-ink-soft mb-1.5">Price / NAV per unit (₹)</label>
                    <input type="number" step="0.0001" placeholder="112.35" value={initialPrice} onChange={(e) => setInitialPrice(e.target.value)} className="field-input" />
                  </div>
                  <div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea rows="3" value={initialNotes} onChange={(e) => setInitialNotes(e.target.value)} className="field-input" placeholder="SIP instalment, broker note, folio reference…" />
                  </div>
