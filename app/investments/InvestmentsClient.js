...
import { inr, fmtDate, TYPE_META, toneFor, labelFor } from '@/lib/format';
      ...
      <div className="px-4 md:px-8 py-5 md:py-6 max-w-4xl mx-auto w-full">
        ...
        <div className="flex justify-between items-end mb-4">
          <div>
            ...
            <p className="text-sm text-ink-soft mt-1">{filtered.length} {filtered.length === 1 ? 'plan' : 'plans'} · {inr(total)} total</p>
          </div>
          <Link href="/investments/new" className="btn-primary py-2 px-3.5 rounded-full text-xs font-medium">+ Add</Link>
        </div>
        ...
        {!loading && investments.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-sm text-ink-soft mb-5">Add your first FD, mutual fund, or other investment.</p>
            ...
            {filtered.map((i, idx) => {
              ...
              return (
                <Link key={i.id} href={`/investments/${i.id}`}
                  className={`flex items-center justify-between p-3.5 hover:bg-paper-tint/50 transition ${idx > 0 ? 'border-t border-edge' : ''}`}>
                  ...
                  <div className="text-right ml-3 flex-shrink-0">
                    <p className="text-sm font-medium">{inr(i.amount)}</p>
