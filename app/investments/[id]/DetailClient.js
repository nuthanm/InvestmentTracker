...
export default function DetailClient({ user, investment: i, documents }) {
  ...
  const onDelete = async () => {
    await fetch(`/api/investments/${i.id}`, { method: 'DELETE' });
        ...
        <div className="flex items-center gap-3 pt-3 border-t border-edge">
          {confirmDelete ? (
            <>
              ...
              <button onClick={onDelete} className="text-sm text-danger font-medium px-3 py-1.5 rounded-lg border border-danger hover:bg-danger-soft transition">Yes, delete</button>
