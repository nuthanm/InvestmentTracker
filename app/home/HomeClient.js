...
export default function HomeClient({ user }) {
  ...
  const totalValue = investments.reduce((s, i) => s + Number(i.maturity_value || i.amount), 0);
  ...
  const maturingSoon = investments.filter(i => {
    if (!i.maturity_date) return false;
    const days = (new Date(i.maturity_date) - new Date()) / (1000 * 60 * 60 * 24);
        ...
        <div className="flex-1 flex items-center justify-center px-4 py-20">
          <div className="text-center max-w-sm anim-fade">
            ...
            <p className="text-sm text-ink-soft mb-6 leading-relaxed">
              Start by adding what you want to save for, or add an investment you already have.
            </p>
        ...
        <div className="px-4 md:px-8 py-5 md:py-6 max-w-5xl mx-auto w-full">
          ...
          <div className="md:flex md:items-end md:justify-between mb-5">
            <div>
              ...
              <p className={`text-sm mt-1.5 ${totalReturns >= 0 ? 'text-mint-600' : 'text-danger'}`}>
                {totalReturns >= 0 ? '+' : ''}{inr(totalReturns)} ({returnPct}%)
              </p>
            </div>
            <Link href="/investments/new" className="hidden md:inline-flex items-center gap-1.5 btn-primary py-2 px-4 rounded-full text-sm font-medium">
              + Add investment
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 mb-6">
            ...
            <Link href="/investments" className="bg-paper-card border border-edge rounded-xl p-3.5 hover:border-mint-600 transition">
              <p className="text-[11px] text-ink-mute">Total invested</p>
          ...
          <div className="grid md:grid-cols-2 gap-4 md:gap-5">
            ...
            <section className="bg-paper-card border border-edge rounded-2xl p-4 md:p-5">
              ...
              {goals.length === 0 ? (
                <Link href="/goals/new" className="block text-center py-6 text-sm text-ink-mute border border-dashed border-edge rounded-xl hover:bg-paper-tint">
                  + Add your first goal
                </Link>
            ...
            <section className="bg-paper-card border border-edge rounded-2xl p-4 md:p-5">
              ...
              {investments.slice(0, 4).map((i) => {
                ...
                return (
                  <Link key={i.id} href={`/investments/${i.id}`} className="flex items-center justify-between py-2.5 border-b border-edge last:border-b-0 hover:bg-paper-tint/50 -mx-2 px-2 rounded transition">
                    <div className="flex gap-2.5 items-center min-w-0 flex-1">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-medium flex-shrink-0 ${TONE_BG[tone]}`}>
                        {TYPE_META[i.type_code]?.short || 'OT'}
                      </div>
                      <div className="min-w-0">
                        <p className="text-xs font-medium truncate">{i.bank} · {i.plan_name}</p>
                        <p className="text-[11px] text-ink-mute mt-0.5">{labelFor(i)} · matures {fmtDate(i.maturity_date)}</p>
                      </div>
                    </div>
                    <div className="text-right ml-2 flex-shrink-0">
                      <p className="text-xs font-medium">{inrShort(i.amount)}</p>
                      <p className="text-[10px] text-mint-600 mt-0.5">{i.rate_pct}% p.a.</p>
                    </div>
                  </Link>
                );
              })}
